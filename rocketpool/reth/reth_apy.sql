...
select
    ...
    avg(apy) over (order by hr asc rows between 24*7 preceding and current row) as apy_7d,
    avg(apy) over (order by hr asc rows between 24*30 preceding and current row) as apy_30d,
    avg(apy) over (order by hr asc rows between 24*90 preceding and current row) as apy_90d,
    avg(apy) over (order by hr asc rows between 24*120 preceding and current row) as apy_120d,
